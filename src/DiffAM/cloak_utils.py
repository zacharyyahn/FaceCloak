...
def get_diffam_config():
    with open("configs/MT.yml", 'r') as f:
        config = yaml.safe_load(f)
    config = dict2namespace(config)
    return config

def get_diffam_model(config, device):
    model_path = "checkpoint/test_MT_CelebA_HQ_XMY-060_t60_ninv20_ngen6_dis1_dir0.3_lr8e-06_XMY-060-3.pth"
    model = DDPM(config)
